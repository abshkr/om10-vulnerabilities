...
// initialize object
$idassign = new EquipmentType($db);

$etyp_title = (isset($_GET["etyp_title"]) ? 
    '%' . $_GET["etyp_title"] . '%' : '%');
$cmptnu = (isset($_GET["cmptnu"]) ? 
    $_GET["cmptnu"]: null);

// query products
$stmt = $idassign->search2($etyp_title, $cmptnu);
 
// products array
$personnels_arr = array();
$personnels_arr['result_count'] = $idassign->searchCount($etyp_title, $cmptnu);
...
if ($num > 0) {
    ...
    echo json_encode(
        array("message" => "No tanker owner record found.")
    );
}
