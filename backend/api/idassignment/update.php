...
if ($data) {
    ...
    if (property_exists($data, 'resetpin')) 
        $idassign->resetpin = $data->resetpin;
    if (property_exists($data, 'removepin')) 
        $idassign->removepin = $data->removepin;
} else {
    ...
    if (isset($_GET["resetpin"]))
        $idassign->resetpin= $_GET["resetpin"];
    if (isset($_GET["removepin"]))
        $idassign->removepin= $_GET["removepin"];
}

// create the idassign
if ($idassign->update()){
    echo '{';
        echo '"message": "id assignment was updated."';
    echo '}';
} else{
    echo '{';
        echo '"message": "Unable to update id assignment."';
    echo '}';
}
