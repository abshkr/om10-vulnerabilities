...
function displayProdItemList(curPrivilage,curColumnToSort)
{
	...
	//Paging row moved to another location dispFrm += makespace("\t", indent) + nextPage(pagesTotal_3, pg_3, "prod_base_prices.php", "pg_3");
	...
	if (is_search_on == 1)
	{
		...
		dispFrm += makespace("\t", indent+1) + "<form name=\"list_prod_base_prices\" method=\"get\" id=\"list_prod_base_prices\" action=\"prod_base_prices.php\" onsubmit=\"return Validator.Validate(this,1);\">\n";
...
//		dispFrm += makefield(1, otherText["search_prodcmpy"], frm_prodcmpy_search, "frm_prodcmpy_search", "frm_prodcmpy_search", "", 30, 7, "onchange=\"submit();\" ", "", "&nbsp;", indent+4, 80);
	...
	dispFrm += makespace("\t", indent) + nextPage(pagesTotal_3, pg_3, "prod_base_prices.php", "pg_3");
...
function displayModifyProdItemForm ()
{
	...
	updFrm += makespace("\t", indent+1) + "<form name=\"edit_prod_base_price\" method=\"get\" id=\"edit_prod_base_price\" action=\"prod_base_prices.php\" onsubmit=\"return Validator.Validate(this,1);\">\n";
...
/*
	if(priv>=7)
	{
		btn_HTML += btnLocation_HTML("justChaneMyLocation('prod_base_prices.php?pg_3='+pg_3+'&pg='+pg+'&suppCd='+suppCd+'&cmpyCd='+cmpyCd+'&custAcc='+custAcc+'&termCd='+termCd+'&orderNo='+orderNo+'&op=" + opValues["insertProdBasePricesForm"] + "'); ", otherText["btn_addLoadDelvQty"]);
	}
*/

	if(priv>=5 && is_search_on != 1)
	{
		btn_HTML += btnLocation_HTML("justChaneMyLocation('prod_base_prices.php?is_search_on=1&pg_3='+pg_3+'&pg='+pg+'&op=" + opValues["listProdBasePrices"] + "'); ", otherText["btn_search_product"]);
	}
	else if(priv>=5 && is_search_on == 1 )
	{
		btn_HTML += btnLocation_HTML("justChaneMyLocation('prod_base_prices.php?op=" + opValues["listProdBasePrices"] + "'); ", otherText["view_all"]);
	}
...
function btnGroupModifyProdBasePrices_HTML ()
{
	...
	btn_HTML += btnLocation_HTML("justChaneMyLocation('prod_base_prices.php?pg_3='+pg_3+'&pg='+pg+'&prodCd='+prodCd+'&prodCmpy='+prodCmpy+'&op=" + opValues["listProdBasePrices"] + "'); ", otherText["btn_bakto_prodBasePrices"]);
