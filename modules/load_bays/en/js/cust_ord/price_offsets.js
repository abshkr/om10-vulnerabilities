	...
	var column_headers = [
			"Code", "Description", "Type", "Application", "Printed"
	];
...
function displayPriceOffsetList(curPrivilage,curColumnToSort)
{
	...
	dispFrm += makespace("\t", indent) + btnGroupPriceOffset_HTML();
	dispFrm += makespace("\t", indent) + nextPage(pagesTotal_3, pg_3, "price_offsets.cgi", "pg_3");
			...
			for(var j=0; j<column_headers.length; j++)
			{
				...
				else 
				{
					...
					if(howmanyDone==0) // means time to display the drop list and table
					{
						...
						dispFrm += makespace("\t", indent+5) + "<input type=\"hidden\" name=\"proffCd\" id=\"proffCd\" value=\"" + price_offset_jstab[i][howmanyDone] + "\">\n";
						...
						dispFrm += makespace("\t", indent+5) + op_list (curPrivilage, price_offset_jstab[i][howmanyDone], i);
					...
					else
					{
						if (howmanyDone == 4)
						{
							dispFrm += makespace("\t", indent+3) + obs( yes_no_array[ price_offset_jstab[i][howmanyDone] ]);
						}
						else
						{
