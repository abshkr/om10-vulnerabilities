...
_bsn.AutoSuggest = function (fldID, param)
{
	...
	if (!this.oP.cache)			this.oP.cache = false; //set it to true if need cache
...
_bsn.AutoSuggest.prototype.setSuggestionsNoList = function (req)
{
...
_bsn.AutoSuggest.prototype.changeHighlight = function(key)
{
	...
	if (this.iHighlighted > 0 && list.childNodes.length>0)
	...
	if (this.iHighlighted > list.childNodes.length && list.childNodes.length>0)
...
_bsn.AutoSuggest.prototype.getSuggestionArr = function ()
{
  /* Abdul did all this to fetch the 
   * latest from the data base in case 
   * carrier has been updated or some other
  ...
  var pointer = this;
	...
	var onSuccessFunc = function (req) { pointer.setSuggestionsNoList(req) };
	var onErrorFunc = function (status) { alert("AJAX error: "+status); };
	...
	if (this.aSuggestions.length>0) return this.aSuggestions;
	return false;
}
