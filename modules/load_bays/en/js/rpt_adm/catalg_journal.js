...
{ 
  ...
  newPage +="<tbody>\n";  
	...
	if (curViewDetailState <= opValues["init"] || curViewDetailState >=15 )
	{
		...
		newPage +=displayDataTable(curPrivilage, cat_journal_tab, curColumnToSort, true );
		newPage += "</tr> \n";		
	...
	if ( parseInt(pg) > 0 && parseInt(pagesTotal) > 1 )
		{
			newPage +=nextPage_longStr(pagesTotal, pg, "pg",'op', op,'days', days,'endDate', endDate, 'startDate',startDate,'statusBar','');
		}
		...
		if(i>0)
		{
			...
			for(var j=1; j<inPutArray[i].length; j++)
			{
				...
				else 
				{
					...
					{	
						if(j==4)	// Display the Bay Or Personnel who modified the key
						{	newPage += "<td>\n";
							if(inPutArray[i][1]=="LLES")//That means RDI Stop button locked key
							{
								if(inPutArray[i][j]!="" && inPutArray[i][j]!="-1") newPage += "<span style=\"COLOR: #FF0000; font-weight:bold;\">"+ml(t__RDI_Stop_Buttn) +"  "+ml(t__Key_Lock)+"<\/span> : "+ml(t__Bay)+": "+(inPutArray[i][j])+" ";
							}
							else if(inPutArray[i][1]=="FAKM")//Key Modified from Front End
							{
								if(inPutArray[i][j]!="" && inPutArray[i][j]!="-1") newPage += "<span style=\"COLOR: orange; font-weight:bold;\">"+ml(t__Key_Updated) +"<\/span> : "+ml(t__Person)+"  "+ml(t__Key_Updated)+": "+(inPutArray[i][j])+" " +(inPutArray[i][j+1])+" ";
							}
							
						}
						else if(j==5)	// Display the Trip
						{
							if(inPutArray[i][j]!="" && inPutArray[i][j]!="-1" && inPutArray[i][1]=="LLES") newPage += ml(t__Trip_No)+": "+(inPutArray[i][j])+" ";
							
						} 					
						else if(j==6)	// Display the Transaction Id Or For Key Updated display status
						{
							if(inPutArray[i][j]!="" && inPutArray[i][j]!="-1" && inPutArray[i][1]=="LLES") newPage += ml(t__Transaction)+": "+(inPutArray[i][j])+" ";
							if(inPutArray[i][j]!="" && inPutArray[i][j]!="-1" && inPutArray[i][1]=="FAKM") 
							{
								newPage += ml(t__Key_Lock)+": ";
								if(inPutArray[i][j]=="Y") newPage +=commText["Yes"]+" ";
								if(inPutArray[i][j]=="N") newPage +=commText["No"]+" ";
							}
							
						}
						else if(j==7)	// Display the Person Code and Name
						{
							if(inPutArray[i][j]!="" && inPutArray[i][j]!="-1") newPage += ml(t__Person)+": "+(inPutArray[i][j])+" "+(inPutArray[i][j+1])+" ";
						}  
						else if(j==9)	// Display the  Tanker
						{
							if(inPutArray[i][j]!="" && inPutArray[i][j]!="-1") newPage += ml(t__Search_Tanker)+": "+(inPutArray[i][j])+" ";
						}
						else if(j==10)	// Display the Issuer company and Name
						{
							if(inPutArray[i][j]!="" && inPutArray[i][j]!="-1") newPage += ml(t__Issuer)+": "+(inPutArray[i][j])+" "+(inPutArray[i][j+1])+" ";
						}
						else if(j==12)	// Display the Tag Id
						{
							if(inPutArray[i][j]!="" && inPutArray[i][j]!="-1") newPage += ml(t__Tag_Id)+": "+(inPutArray[i][j])+" ";
							newPage += "<\/td>\n";
...
function findForm()
{
  ...
  findFrm += "<table>\n";  
  findFrm += "<tr>\n";
  findFrm += "<td class=\"infotextheading\" width=\"280\">\n";
