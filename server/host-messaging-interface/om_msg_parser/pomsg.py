...
def __get_cmd_line_arguments():
	...
	cmd_arg_parser.add_option('-p', dest='preprocess', help='pre process data file with this script first')
	cmd_arg_parser.add_option('-q', dest='postprocess', help='post process data file with this script last')
...
def __postprocess(script_file, data_str, dest_file):
	if script_file != None and os.path.isfile(script_file):
		now = datetime.now()
		data_file = now.strftime("%Y%m%d%H%M%S") + '.xml'
...
if __name__ == "__main__":
	...
	(post,res_file) = __postprocess(options.postprocess, msg_str, options.output_file)
