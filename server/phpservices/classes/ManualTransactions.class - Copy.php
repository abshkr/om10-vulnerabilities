    ...
    public function to_string()
    {
        ...
        for ($i = 0; $i < $this->Number_of_Compartments; ++ $i)
        {
            $msg_string = $msg_string . 
                $this->cmpt_det[$i]->eqp_cd . 
                $this->cmpt_det[$i]->nr_in_eqp . 
                $this->cmpt_det[$i]->nr_in_tkr . 
                $this->cmpt_det[$i]->Volume_Safe_Fill_Limit .
                $this->cmpt_det[$i]->Mass_Limit . 
                $this->cmpt_det[$i]->Volume_Capacity;

            $msg_string = $msg_string . $this->cmpt_det[$i]->prod_def;
            $msg_string = $msg_string . $this->cmpt_det[$i]->asmx;
            $msg_string = $msg_string . $this->cmpt_det[$i]->Nr_of_Drums_Specified;
            $msg_string = $msg_string . $this->cmpt_det[$i]->prod_class1;
            $msg_string = $msg_string . $this->cmpt_det[$i]->hybrid_loading;
            $msg_string = $msg_string . $this->cmpt_det[$i]->prod_def2;
            $msg_string = $msg_string . $this->cmpt_det[$i]->asm2;
            $msg_string = $msg_string . $this->cmpt_det[$i]->prod_class2;
            $msg_string = $msg_string . $this->cmpt_det[$i]->temps1;
            $msg_string = $msg_string . $this->cmpt_det[$i]->asm3;
            $msg_string = $msg_string . 
                $this->cmpt_det[$i]->NrBases1 .
                $this->cmpt_det[$i]->Nr_of_Preloaded_Drums;
            $msg_string = $msg_string . $this->cmpt_det[$i]->prod_class3;
            $msg_string = $msg_string . $this->cmpt_det[$i]->temps2;
            $msg_string = $msg_string . $this->cmpt_det[$i]->asm4;

            $msg_string = $msg_string . $this->cmpt_det[$i]->NrBases2;
            for ($j = 0; $j < $this->cmpt_det[$i]->NrBases2; ++$j)
            {
                $msg_string = $msg_string . $this->cmpt_det[$i]->qty2[$j];
            }
            
            $msg_string = $msg_string . $this->cmpt_det[$i]->prod_class4;
            $msg_string = $msg_string . $this->cmpt_det[$i]->temps3;
            $msg_string = $msg_string . $this->cmpt_det[$i]->asm5;
            $msg_string = $msg_string . 
                $this->cmpt_det[$i]->NrBases3 .
                $this->cmpt_det[$i]->Nr_of_Loaded_Drums . 
                $this->cmpt_det[$i]->Prompt_Flags;
        }
        ...
        {
            $msg_string = $msg_string . 
                $this->trf_det[$i]->Transfer_Number . 
                $this->trf_det[$i]->Arm_Code . 
                $this->trf_det[$i]->Device_Code . 
                $this->trf_det[$i]->eqp_cd .
                $this->trf_det[$i]->nr_in_eqp . 
                $this->trf_det[$i]->nr_in_tkr;

            $msg_string = $msg_string . $this->trf_det[$i]->pdx;
            $msg_string = $msg_string . $this->trf_det[$i]->pdy;
            $msg_string = $msg_string . $this->trf_det[$i]->asmx;
            $msg_string = $msg_string . $this->trf_det[$i]->Nr_of_Drums;
            $msg_string = $msg_string . $this->trf_det[$i]->prod_class;
            $msg_string = $msg_string . $this->trf_det[$i]->temps;
            $msg_string = $msg_string . $this->trf_det[$i]->asm2;
            $msg_string = $msg_string . $this->trf_det[$i]->prod_class1;
            $msg_string = $msg_string . $this->trf_det[$i]->temps2;
            $msg_string = $msg_string . $this->trf_det[$i]->asm3;
                
            $msg_string = $msg_string . $this->trf_det[$i]->Number_of_Meters;
            for ($j = 0; $j < $this->trf_det[$i]->Number_of_Meters; ++$j)
            {
                $msg_string = $msg_string . 
                    $this->trf_det[$i]->mtr_det[$j]->Injector_or_Meter . 
                    $this->trf_det[$i]->mtr_det[$j]->Meter_Injector_Code;

                $msg_string = $msg_string . $this->trf_det[$i]->mtr_det[$j]->pd;
                $msg_string = $msg_string . $this->trf_det[$i]->mtr_det[$j]->prod_class;
                $msg_string = $msg_string . $this->trf_det[$i]->mtr_det[$j]->temps;
                $msg_string = $msg_string . $this->trf_det[$i]->mtr_det[$j]->asmx;
                $msg_string = $msg_string . $this->trf_det[$i]->mtr_det[$j]->asm2;
                $msg_string = $msg_string . $this->trf_det[$i]->mtr_det[$j]->asm3;
                $msg_string = $msg_string . $this->trf_det[$i]->mtr_det[$j]->pd;
                $msg_string = $msg_string . 
                    $this->trf_det[$i]->mtr_det[$j]->Start_Mass . 
                    $this->trf_det[$i]->mtr_det[$j]->End_Mass;
            }
        
            $msg_string = $msg_string . $this->trf_det[$i]->Number_of_Bases;
            for ($j = 0; $j < $this->trf_det[$i]->Number_of_Bases; ++$j)
            {
                $msg_string = $msg_string . $this->trf_det[$i]->base[$j];
            }
        
            $msg_string = $msg_string . 
                $this->trf_det[$i]->Start_Mass . 
                $this->trf_det[$i]->End_Mass . 
                $this->trf_det[$i]->Was_Anything_Recycled;
        }
...
class ManualTransactions
{
    ...
    private function has_other_active_trip($tanker, $current_trip)
    {
        ...
        else
        {
            $sql = "SELECT SHLS_TRIP_NO FROM SCHEDULE, LOADS WHERE SHL_TANKER = '" . $tanker . "' AND SHLSLOAD_LOAD_ID = LOAD_ID
                AND LD_TERMINAL = SHLSLOAD_LD_TRM AND LOAD_DMY IS NULL";
            $result = $this->db_conn->query($sql);
            logMe($result[0]->SHLS_TRIP_NO, $current_trip);
    ...
    {
        $sql = "SELECT SHL_TANKER, NVL(STATS, 'N') STATS FROM SCHEDULE WHERE SHLS_TRIP_NO = " . $trip . " AND SHLS_SUPP = '" . $supplier . "'";
        $result = $this->db_conn->query($sql);
    ...
    {
        $sql = "SELECT SHLS_TRIP_NO FROM SCHEDULE, MOV_SCHEDULES, MOVEMENT_ITEMS WHERE STATS = 'L' " . 
    ...
    private function populate_transa_det($order_number, $para_trans, $num_of_transfers, $transfers, $is_nomination = 0, $auto_complete = "F", &$trans_id = 0)
    {
        //Sample:
        ...
        if ($trans_id == 0)
            $trans_id = $this->get_trnsid();
        $msg->Transaction_Number = sprintf("%07d", $trans_id);   /* Get it from parameter, sample: "0010007" */
        ...
        $msg->Drawer_Code = sprintf("%-16.16s", $drawer_code);     /* Get it from parameter, sample: "1001            " */
        $msg->Drawer_Name = sprintf("%-50.50s", $drawer_name);     /* Get it from parameter, sample: "SHELLSAP                                          " */
        $msg->Tanker_Code = sprintf("%-20.20s", $tanker_Code);     /* Get it from parameter, sample: "H80303FG            " */
        if ($this->isPreOrderSchedule($para_trans->Supplier, $schedule_id) == "YES")
        {
            $sql_vet = "SELECT C.CMPY_VET FROM COMPANYS C WHERE C.CMPY_CODE='" . $para_trans->Supplier. "'";
            $rows_vet = $this->db_conn->query($sql_vet);
            //logMe("rows_vet[0]->CMPY_VET ===>".$rows_vet[0]->CMPY_VET, Manual_TRANSACTION);
            if ($rows_vet[0]->CMPY_VET != 1 && $rows_vet[0]->CMPY_VET != 2 && $rows_vet[0]->CMPY_VET != 3) 
            {
                $msg->Tanker_Code = sprintf("%-20.20s", $para_trans->Tanker_Code);
            }
        }
        //logMe("Outgoing tanker code " . $msg->Tanker_Code, Manual_TRANSACTION);
        ...
        for ($i = 0; $i < $num_of_compartment; ++ $i)
        {
            ...
            $msg->cmpt_det[$i]->eqp_cd = 
                sprintf("%-20.20s", $compartments[$i]->eqp_cd); /* Get it from parameter, sample: "KS037               " */
        ...
        for ($i = 0; $i < $num_of_transfers; ++$i)
        {
            ...
            $msg->trf_det[$i]->Arm_Code = sprintf("%-6.6s", $transfers[$i]->Arm_Code);            /* Sample: "A00101" */
            ...
            for ($j = 0; $j < $transfers[$i]->num_of_meter; ++$j)
            {
                ...
                $msg->trf_det[$i]->mtr_det[$j]->Meter_Injector_Code =
                    sprintf("%-16.16s", $transfers[$i]->meters[$j]->Meter_Injector_Code); /* Sample: "101             " */
    ...
    function do_nomination($mv_item_id, $para_trans, $num_of_transfers, $transfers, $from = null, $to = null, $alt_obj = null)
    {
        $trans_id = $this->get_trnsid();
        ...
        if ($this->get_mvitem_type($mv_item_id) == MV_DISPOSAL)
        {
            if ($from)
            {
                if ($from->tank)
        ...
        else if ($this->get_mvitem_type($mv_item_id) == MV_RECEIPT)
        {
            if ($to)
            {
        ...
        else if ($this->get_mvitem_type($mv_item_id) == MV_TRANSFER)
        {   
            if ($to)    /* If it is transfer, handle unloading first. This is consistent with baiman, because for transfer, baiman handles unloading first */
            {
                $transfers[0]->bases[0]->Tank_Code = $to->tank;
                $para_trans->Supplier = $to->supplier;
                $para_trans->Drawer_Code = $to->supplier;
        ...
        $tran_det = $this->populate_transa_det(0, $para_trans, $num_of_transfers, $transfers, $is_nomination, "T", $trans_id);
    ...
    function do_create($order_number, $para_trans, $num_of_transfers, $transfers, $is_nomination = 0, $auto_complete = "F", $trans_id = 0)
    {
        ...
        {   /* Open order loading */
            ...
            /* 2. Trasaction dets*/
            $tran_det = $this->populate_transa_det($order_number, $para_trans, $num_of_transfers, $transfers, $is_nomination, $auto_complete, $trans_id);
            $client->send($tran_det);
            $response = $client->get_repond();
        ...
        else
        {
            ...
            $php_commit = false;
            $vet = $this->switch_tanker($para_trans->Load_Number, $para_trans->Supplier, $para_trans->Tanker_Code, $result_detail->result_string);
            if ($vet < 0)
            {
                $result_detail->result_code = -1;
                return $result_detail;
            ...
            $trans_id= 0;
            $tran_det = $this->populate_transa_det(0, $para_trans, $num_of_transfers, $transfers, 0, $auto_complete, $trans_id);
            $client = new socket_client($this->bay_code);
            $client->send($tran_det);
            $response = $client->get_repond();
            
            $reply->Result_Code = substr($response, 75, 10);
            if ($reply->Result_Code != null && substr_compare($reply->Result_Code, "OK", 0, 2) == 0)   //Once it's created, the stored file can be deleted (if do_save() has once been called)
            {
                // Added $reply->Result_Code != null check to avoid Front-end channel disconnect error.
                // If error occurred in baiman, Result_Code coulde be null, which will cause substr_compare failed.
                // 31/Oct/2013
                ...
                if ($para_trans->Login_User)
                {
                    $sql = "UPDATE SCHEDULE SET OPERATOR = '" . $para_trans->Login_User . "' , LAST_CHG_TIME = SYSDATE WHERE SHLS_TRIP_NO = " . $para_trans->Load_Number . " AND SHLS_SUPP = '" . $para_trans->Supplier . "'";
                    if ($this->db_conn->update($sql) == RETURN_OK)
                    {
                        $php_commit = true;
                        $sql = "UPDATE TRANSACTIONS SET TRSA_PSN = '" . $para_trans->Login_User . "' WHERE TRSA_ID = " . $trans_id;
    ...
    private function initial_speicalmovement($type, $mlitm_id, &$amb, &$cor, &$kg, &$temp, &$dens, &$drawer, &$prod, &$tank, &$start, &$end)
    {
    ...
    function do_special($mlitm_id)
    {
        ...
        $trans->Supplier = trim(substr($response, 50, 20));
        ...
        if ($this->initial_speicalmovement(MV_RECEIPT, $mlitm_id, $amb, $cor, $kg, $temp, $dens, $drawer, $prod, $tank, $start, $end) != 0)
        {
            $result_detail->result_code = -1;
            $result_detail->result_string = "WRONG SCHEDULE";
            return $result_detail;
        ...
        $trans = new Manual_Transa();
        //$trans->Transaction_Number = "0020029";       //"0010007" is the sample value
        $trans->Load_Number = trim(substr($response, 41, 9));              //"4373229"
        $trans->Supplier = trim(substr($response, 50, 20));
