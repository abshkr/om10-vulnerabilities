...
//if(!defined('Manual_TRANSACTION'))
define('REVERSE_TRANSACTION', 'reverse_trans.class');
//main();
...
class socket_client
{
    ...
    public function __construct($bay = "BAY999" /*By default, use BAY999 as the manual transaction server bay */)
    {
...
class ReverseTransactionClass
{
    ...
    public function do_create($order_number, $para_trans, $num_of_transfers, $transfers)
    {
        if ($order_number != 0) { /* Open order loading */
            /* 1. auth */
    ...
    public function do_specmov_reverse($mlitm_id)
    {
        $sql = "SELECT MSITM_SHLSTRIP, MSITM_SHLSSUPP FROM MOV_SCHD_ITEMS, MOV_LOAD_ITEMS WHERE MSITM_MOVEID =MLITM_MOV_ID  AND MSITM_MOVITEM = MLITM_MOV_LINE AND MLITM_ID = " . $mlitm_id;
        $result = $this->db_conn->query($sql);
        $count = count($result);
        if ($count > 2) { /* Unlikely to happen, one special movment should have only one trip (or 2, if it is a tranfser ) */
            $result_detail->result_code = -1;
            $result_detail->result_string = "More than one trip found for this special movement";
        } else if ($count <= 0) {
            $result_detail->result_code = -1;
            $result_detail->result_string = "No trip is linked to this special movement";
        } else {
            for ($i = 0; $i < $count; ++$i) {
        ...
        if ($result_detail->result_code == 0) {
            $sql = "UPDATE MOV_LOAD_ITEMS SET MLITM_STATUS = 9 WHERE MLITM_ID = " . $mlitm_id;
            $res = $this->db_conn->update($sql);
            if (RETURN_OK == $res) {
                $this->db_conn->commit();
            } else {
    ...
    public function do_reverse($trip_id, $supplier, $login_user)
    {
        ...
        if (substr_compare($response, "OK", 12, 2) == 0) {
            $result_detail->result_code = 0;
            $result_detail->result_string = "Schedule reversed successfully";

            $sql = "UPDATE SCHEDULE SET OPERATOR = '" . $login_user . "', LAST_CHG_TIME = SYSDATE WHERE SHLS_TRIP_NO = " . $trip_id . " AND SHLS_SUPP = '" . $supplier . "'";
            if ($this->db_conn->update($sql) == RETURN_OK) {
                //$this->db_conn->commit();
                $sql = "SELECT MAX(TRSA_VERSION) MAX_VERSION FROM TRANSACTIONS WHERE (TRSALDID_LD_TRM, TRSALDID_LOAD_ID) IN (SELECT SHLSLOAD_LD_TRM, SHLSLOAD_LOAD_ID FROM SCHEDULE WHERE SHLS_TRIP_NO = " .
                    $trip_id . " AND SHLS_SUPP = '" . $supplier . "')";
                $result = $this->db_conn->query($sql);
                $sql = "UPDATE TRANSACTIONS SET TRSA_PSN = '" . $login_user . "' WHERE (TRSALDID_LD_TRM, TRSALDID_LOAD_ID) IN (SELECT SHLSLOAD_LD_TRM, SHLSLOAD_LOAD_ID FROM SCHEDULE WHERE SHLS_TRIP_NO = " .
                $trip_id . " AND SHLS_SUPP = '" . $supplier . "') AND TRSA_VERSION = " . $result[0]->MAX_VERSION;
    ...
    public function do_save($para_trans, $num_of_transfers, $transfers)
    {
    ...
    public function do_load($trns_id, &$para_trans, &$num_of_transfers, &$transfers)
    {
    ...
    public function validate_before_rev_archive($trip, $supplier, &$cmpy_code)
    {
        $sql = "SELECT CMPY_CODE FROM COMPANYS WHERE CMPY_CODE =
        ...
        if ($res_count == 1) {
            $cmpy_code = $result[0]->CMPY_CODE;
            logMe("validate_before_rev_archive() Good.", Manual_TRANSACTION);
            return "OK";
        } else {
            logMe("validate_before_rev_archive() BAD.", Manual_TRANSACTION);
            return "FAIL";
        }
    }

    public function do_reverse_archive($trip, $supplier, $login_user)
    {
        ...
        if ($this->validate_before_rev_archive($trip, $supplier, $cmpy_code) == "OK") {
            $sql = "
            SELECT L.LOAD_REVERSE_FLAG FROM LOADS L, SCHEDULE S
            ...
            if ($res_count < 1) {
                $result_detail->result_code = -1;
                $result_detail->result_string = "Could not find closed Schedule." . $res_count;
                return $result_detail;
            }

            if ($result[0]->LOAD_REVERSE_FLAG != 1) {
                logMe("Rev/Archive: do reverse first.", Manual_TRANSACTION);

                /*step: do reverse */
                $result_detail = $this->do_reverse($trip, $supplier, $login_user);
                if ($result_detail->result_code != 0) {
                    logMe("Rev/Archive: do reverse failed.", Manual_TRANSACTION);
                    return $result_detail;
                }
                logMe("Rev/Archive: Completed reversal.", Manual_TRANSACTION);
            }

            logMe("Rev/Archive: start archiving ...", Manual_TRANSACTION);
            /*step: call procedure to complete */
            $rsult = -1;
            $rev_arch = oci_parse($this->db_conn->connect, "BEGIN archive_trip(:i_trip_no_old, :i_supp_code_old, :i_supp_code_new, :o_exec_rst); END;");
            oci_bind_by_name($rev_arch, ":i_trip_no_old", $trip);
            oci_bind_by_name($rev_arch, ":i_supp_code_old", $supplier);
            oci_bind_by_name($rev_arch, ":i_supp_code_new", $cmpy_code);
            oci_bind_by_name($rev_arch, ":o_exec_rst", $rsult);

            oci_execute($rev_arch);

            if ($rsult == 0) {
                $result_detail->result_code = 0;
                $result_detail->result_string = "Complete Rev/Archive successfully.";

                $sql = "UPDATE SCHEDULE SET OPERATOR = '" . $login_user . "' , LAST_CHG_TIME = SYSDATE WHERE SHLS_TRIP_NO = " . $trip . " AND SHLS_SUPP = '" . $supplier . "'";
                $this->db_conn->update($sql);

                $sql = "SELECT MAX(TRSA_VERSION) MAX_VERSION FROM TRANSACTIONS WHERE (TRSALDID_LD_TRM, TRSALDID_LOAD_ID) IN (SELECT SHLSLOAD_LD_TRM, SHLSLOAD_LOAD_ID FROM SCHEDULE WHERE SHLS_TRIP_NO = " .
                    $trip . " AND SHLS_SUPP = '" . $supplier . "')";
                $result = $this->db_conn->query($sql);
                $sql = "UPDATE TRANSACTIONS SET TRSA_PSN = '" . $login_user . "' WHERE (TRSALDID_LD_TRM, TRSALDID_LOAD_ID) IN (SELECT SHLSLOAD_LD_TRM, SHLSLOAD_LOAD_ID FROM SCHEDULE WHERE SHLS_TRIP_NO = " .
                $trip . " AND SHLS_SUPP = '" . $supplier . "') AND TRSA_VERSION = " . $result[0]->MAX_VERSION;

                $this->db_conn->commit(); /* ???? Todo: check return result */

                logMe("Rev/Archive: Completed archive Successfully. Result:" . $rsult, Manual_TRANSACTION);
            } else {
                $result_detail->result_code = -1;
                $result_detail->result_string = "Failed to complete Rev/Archive.";

                $this->db_conn->rollback();

                logMe("Rev/Archive: Failed to complete archive. Result:" . $rsult, Manual_TRANSACTION);
            }

            oci_free_statement($rev_arch);
        } else {
            $result_detail->result_code = -1;
            $result_detail->result_string = "Archive supplier company not set up properly.";

            logMe("Rev/Archive: Archive supplier company not set up properly.", Manual_TRANSACTION);
        }

        logMe("Rev/Archive: Return result: " . $result_detail->result_code . ", " . $result_detail->result_string, Manual_TRANSACTION);
        return $result_detail;
    }
}
