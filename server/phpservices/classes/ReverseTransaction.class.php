...
class ReverseTransactionClass
{
    ...
    function do_specmov_reverse($mlitm_id)
    {
        ...
        $count = count($result);
        if ($count > 2)
        {   /* Unlikely to happen, one special movment should have only one trip (or 2, if it is a tranfser ) */
	...
	function validate_before_rev_archive($trip, $supplier, &$cmpy_code)
	{
	...
	function do_reverse_archive($trip, $supplier, $login_user)
	{
