...
if (!defined('PERSONCLASS')) {
    define('PERSONCLASS', 'Personnel.class');
}
...
class PersonnelClass
{
    ...
    public function create($perData, $perArray)
    {
        ...
        if ($this->updateEmail($perData->per_code, $perData->per_auth, $perData->user_login_count) === -1) {
            logMe("Failed to update email address",
    ...
    private function FA2_enabled($mydb)
    {
        $sql = array();
        $sql['sql_text'] = "SELECT NVL(MAX(CONFIG_VALUE), 'N') FA2_ENABLED FROM SITE_CONFIG WHERE CONFIG_KEY = 'SITE_2FA_ENABLED'";
        $rows = $mydb->query($sql);
        return $rows[0]->FA2_ENABLED === 'Y';
    }

    /** Email address should match SITE_2FA_DOMAINS if it is set
     * DRIVER, CONTRACTOR or LOADER are exclued from this rule
     * return RETURN_OK: OK; -1: error
     */
    private function updateEmail($per_code, $per_role, $per_mail)
    {
        logMe(sprintf("%s::%s START. per_code:%s, per_role:%s, per_mail:%s",
            __CLASS__, __FUNCTION__, $per_code, $per_role, $per_mail),
            sprintf("%s:%d", basename(__FILE__), __LINE__));

        $mydb = DB::getInstance();

        $sql = array();
        $sql['sql_text'] = "SELECT NVL(MAX(CONFIG_VALUE), '-1') FA2_MAILDOMAIN FROM SITE_CONFIG WHERE CONFIG_KEY = 'SITE_2FA_DOMAINS'";
        $rows = $mydb->query($sql);
        $domain_str = $rows[0]->FA2_MAILDOMAIN;

        if ($per_mail !== "" &&
            $domain_str !== '-1' &&
            $this->FA2_enabled($mydb) &&
            !in_array(intval($per_role), array(7, 8, 9)) /** 7: VISITOR; 8: CONTRACTOR; 9: LOADER */) {
            $domain_check = false;
            $domain_array = explode(",", $domain_str);
            foreach ($domain_array as $domain) {
                $domain = trim($domain);
                if (strpos($per_mail, $domain) > 0) {
                    $domain_check = true;
                    break;
                }
            }
            ...
            if ($domain_check === false) {
                $msg = sprintf("Cannot set email for personnel due to mismatch email domain. email:%s, domain:%s", $per_mail, $domain_str);
                $myjournal = new JournalClass(null, null);
                $journal_data = new stdClass();
                $journal_data->company_code = $_SESSION['COMPANY'];
                $journal_data->region_code = 'ENG';
                $journal_data->msg_event = 'CONF';
                $journal_data->msg_class = 'EVENT';
                $journal_data->message = $msg;
                $myjournal->create($journal_data);
                logMe($msg, sprintf("%s:%d", basename(__FILE__), __LINE__));
                return -1;
            }
        }

        $sql['sql_text'] = "UPDATE PERSONNEL SET PER_EMAIL=:psnl_email WHERE PER_CODE=:psnl_code ";
        $sql['sql_data'] = array($per_mail, $per_code);
