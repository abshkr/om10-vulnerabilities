...
if(!defined('CUSTORDER')) define('CUSTORDER','CustOrders.class');

class CustOrdersClass{
	var $tbl_name = "GUI_ORDERS";
	
     public function CustOrdersClass()
    {
        if(defined('HOST')) {
            $this->host = HOST;
        }
    ...
    public function getPaged($offset,$tot,$filter='',$sort='')
	{
		...
		$sql="
			select
				...
				, ORDER_SUPP_CODE
				, ORDER_SUPP_NAME
				, DECODE(ORDER_ORD_TIME, NULL, '', TO_CHAR(ORDER_ORD_TIME, 'YYYY-MM-DD HH24:MI:SS')) as ORDER_ORD_TIME
				, ORDER_TTYP_ID
				, ORDER_TTYP_NAME
				, ORDER_STAT_ID
				, ORDER_STAT_NAME
				...
				, ORDER_DLOC_NAME
				, ORDER_REF_CODE
				, DECODE(ORDER_DLV_TIME, NULL, '', TO_CHAR(ORDER_DLV_TIME, 'YYYY-MM-DD')) as ORDER_DLV_TIME
				, ORDER_DTRM_CODE
				, ORDER_DTRM_NAME
				, DECODE(ORDER_EXP_TIME, NULL, '', TO_CHAR(ORDER_EXP_TIME, 'YYYY-MM-DD')) as ORDER_EXP_TIME
				, ORDER_DRWR_CODE			
				, ORDER_DRWR_NAME
				, ORDER_CARR_CODE
				, ORDER_CARR_NAME
				...
				, ORDER_LIMIT
				, ORDER_APP_NO
				, DECODE(ORDER_APP_TIME, NULL, '', TO_CHAR(ORDER_APP_TIME, 'YYYY-MM-DD')) as ORDER_APP_TIME
				, ORDER_TRSF_TYPE
				, NVL(ORDER_PRICE_PRINTED, 'N') as ORDER_PRICE_PRINTED 
				, ORDER_STRM_CODE
				, ORDER_STRM_NAME
				, ORDER_PAY_NOTE
				, ORDER_PSNL_CODE
				...
				, ORDER_SOLD_TO_NUM
				, ORDER_SHIP_TO_NUM
				, DECODE(ORDER_LAST_CHANGE, NULL, '', TO_CHAR(ORDER_LAST_CHANGE, 'YYYY-MM-DD HH24:MI:SS')) as ORDER_LAST_CHANGE
				, RN
			from
				(
                SELECT res.*, ROW_NUMBER() over ($sort) RN
                FROM(SELECT * FROM GUI_ORDERS $filter) res
				)
        ...
        $arr = array();
        foreach($rows as $x){
			$instructions = $this->getOrderInstructionsAsString( $x->ORDER_SYS_NO, -1 );
			$x->order_instructions = $instructions;
            //$items = $this->getOrderItems( $x->ORDER_SYS_NO );
            //$x->order_items = $items;
            //$schedules = $this->getOrderSchedules( $x->ORDER_SYS_NO );
            //$x->order_schedules = $schedules;
            $arr[] = $x;
        }
        
        //XarrayEncodingConversion($arr);
        return (prepareForAMF($arr, array(0 => "GUI_ORDERS")));        
	}
         
    public function getPagedReport($offset,$tot,$filter='',$sort='')
	{
        $mydb = DB::getInstance();
        if($sort!='')$sort="ORDER BY $sort";
		else $sort = "ORDER BY ORDER_SYS_NO DESC";
        ...
        //XarrayEncodingConversion($arr);
        return ( json_encode( prepareForAMF($arr, array(0 => "GUI_ORDERS")) ) );        
	}
         
    public function getReports($filter='',$sort='')
	{
        $mydb = DB::getInstance();
        if($sort!='')$sort="ORDER BY $sort";
		else $sort = "ORDER BY ORDER_SYS_NO DESC";
		...
		$sql="
			...
			FROM GUI_ORDERS $filter $sort";

        $rows = $mydb->query($sql);
        //$rows = $mydb->report($sql);
		//error_log( "\n".$sql, 3, "temp.log");
    ...
    public function getOrderInstructions( $order_id, $counter )
	{
        ...
        //XarrayEncodingConversion($arr);
        return (prepareForAMF($arr, array(0 => "OrderInstructions")));        
	}
    ...
    public function getOrderSchedules( $order_id )
	{
        ...
        //XarrayEncodingConversion($arr);
        return (prepareForAMF($arr, array(0 => "OrderScheduleLookup")));        
	}
    ...
    public function getOrderPeriods( $order_id, $prod_code, $prod_cmpy )
	{
        ...
        //XarrayEncodingConversion($arr);
        return (prepareForAMF($arr, array(0 => "GUI_ORDER_PERIODS")));        
	}

    public function isSingleOrderToOneTripCmpt()
	{
		//select count(*) from SITE_CONFIG where CONFIG_KEY='OO_TO_ONE_TRIP' and CONFIG_VALUE='S'	
        $g = new GlobalClass();
		$tbl_name = "SITE_CONFIG";
		$filter = " where CONFIG_KEY='OO_TO_ONE_TRIP' and CONFIG_VALUE='S' ";
    ...
    public function getOrderItemSchedules( $order_id, $prod_code, $prod_cmpy )
	{
        ...
        //XarrayEncodingConversion($arr);
        return (prepareForAMF($arr, array(0 => "OrderItemScheduleLookup")));        
	}
    ...
    public function initOrderPeriods( $prod_code, $prod_cmpy )
	{
		...
		$sql="
			select
				NULL as OPRD_ORDER_ID
				, PROD_CODE as OPRD_PROD_CODE
				, PROD_CMPY as OPRD_PROD_CMPY
				, PROD_NAME as OPRD_PROD_NAME
				, NULL OPRD_DRWR_NAME
				, NULL as OPRD_PERIOD_NO
				, NULL as OPRD_PERIOD_START
				, NULL as OPRD_PERIOD_END
				, 5 as OPRD_PROD_UNIT
				, NULL as OPRD_UNIT_NAME
				, 0 as OPRD_PROD_QTY
				, 0 as OPRD_PROD_USED
				, 'N' as OPRD_PRICE_FIXED
				, NULL as OPRD_PROD_PRICE
			from
				PRODUCTS
			where 
				PROD_CODE='$prod_code'
				and PROD_CMPY='$prod_cmpy'  
		";
        $rows = $mydb->query($sql);
		//error_log( "\n".$sql, 3, "temp.log");
    ...
    public function lookupOrderCustomer($supplier="-1")
	{
        $mydb = DB::getInstance();
        $sql="
			select 
        ...
        return (prepareForAMF($rows, array(0 => 'OrderCustomerLookup')));
    }

    public function lookupOrderCompany($type=-1)
	{
		if ($type == -1)
		{
			$bitAnd = 255;
        ...
        //XarrayEncodingConversion($rows);
        return (prepareForAMF($rows, array(0 => 'OrderCmpyLookup')));
    }

    public function lookupCompatibleDrawer($cmpy=-1, $prod=-1)
	{
        $mydb = DB::getInstance();
        $sql="
 			select CMPY_CODE, CMPY_NAME, CMPY_TYPE 
    ...
    public function lookupOrderMethodType()
	{
        ...
        $rows = $mydb->query($sql);
        //XarrayEncodingConversion($rows);
        return (prepareForAMF($rows, array(0 => 'OrderMethodTypeLookup')));
    }
    ...
    public function lookupOrderTrnsptType($ord_mthd)
	{
        ...
        $rows = $mydb->query($sql);
        //XarrayEncodingConversion($rows);
        return (prepareForAMF($rows, array(0 => 'OrderTrnsptTypeLookup')));
    }
    ...
    public function lookupOrderStatusType()
	{
        ...
        $rows = $mydb->query($sql);
        //XarrayEncodingConversion($rows);
        return (prepareForAMF($rows, array(0 => 'OrderStatusTypeLookup')));
    }
    ...
    public function lookupOrderSaleType()
	{
        ...
        $rows = $mydb->query($sql);
        //XarrayEncodingConversion($rows);
        return (prepareForAMF($rows, array(0 => 'OrderSaleTypeLookup')));
    }
    ...
    public function lookupOrderDelvLocation()
	{
        ...
        $rows = $mydb->query($sql);
        //XarrayEncodingConversion($rows);
        return (prepareForAMF($rows, array(0 => 'OrderDelvLocLookup')));
    }
    ...
    public function lookupOrderTerminal()
	{
        ...
        $rows = $mydb->query($sql);
        //XarrayEncodingConversion($rows);
        return (prepareForAMF($rows, array(0 => 'OrderTerminalLookup')));
    }
	
    public function lookupOrderPsnl($employer="-1", $role=-1)
	{
        $mydb = DB::getInstance();
        $sql="
			select 
        ...
        $rows = $mydb->query($sql);
        //XarrayEncodingConversion($rows);
        return (prepareForAMF($rows, array(0 => 'OrderPsnlLookup')));
    }
    ...
    public function lookupOrderErpType()
	{
        ...
        $rows = $mydb->query($sql);
        //XarrayEncodingConversion($rows);
        return (prepareForAMF($rows, array(0 => 'OrderErpTypeLookup')));
    }
    ...
    public function lookupOrderUnit()
	{
        ...
        $rows = $mydb->query($sql);
        //XarrayEncodingConversion($rows);
        return (prepareForAMF($rows, array(0 => 'OrderUnitLookup')));
    }
    ...
    public function lookupOrderPriceType()
	{
        ...
        $rows = $mydb->query($sql);
        //XarrayEncodingConversion($rows);
        return (prepareForAMF($rows, array(0 => 'OrderPriceTypeLookup')));
    }
    ...
    public function lookupOrderPriceOffset()
	{
        ...
        $rows = $mydb->query($sql);
        //XarrayEncodingConversion($rows);
        return (prepareForAMF($rows, array(0 => 'OrderPriceOffsetLookup')));
    }
	
    public function lookupOrderProduct($drawer="-1")
	{
        $mydb = DB::getInstance();
        $sql="
			select 
        ...
        $rows = $mydb->query($sql);
        //XarrayEncodingConversion($rows);
        return (prepareForAMF($rows, array(0 => 'OrderProductLookup')));
    }
	
    public function lookupOrderTrip($supplier="-1", $status="-1")
	{
        $mydb = DB::getInstance();
        $sql="
			select 
        ...
        $rows = $mydb->query($sql);
        //XarrayEncodingConversion($rows);
        return (prepareForAMF($rows, array(0 => 'OrderTripLookup')));
    }
