...
if (!defined('COMPCLASS')) {
    define('COMPCLASS', 'Companies.class');
}
...
class CompaniesClass
{
    ...
    public function getCompanies($filter = '', $sort = '')
    {
        ...
        $sql['sql_text'] = "select
            ...
            CMPY_PERMIT_NO
        from GUI_COMPANYS " . $filter['sql_text'] . " " . $sort;
    ...
    public function getPaged($offset, $tot, $filter = '', $sort = '')
    {
        ...
        logMe("Sort: " . $sort, COMPCLASS);
        if ($key == '') {
            $key = getTableKey("companys");
        }

        if ($sort != '') {
            $sort = "ORDER BY nlssort($sort,'NLS_SORT=BINARY_CI') ASC";
        }
