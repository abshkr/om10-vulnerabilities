...
prompt set new configurations to determine the default date ranges in the screen of load schedules, open orders, transactions, live journals, allocations, auditing data, and movement nominations
...
prompt set a new configuration to determine if open orders are filtered by expiry date or ordered date

delete from SITE_CONFIG where CONFIG_KEY='SITE_OO_FILTER_BY_EXPIRY';
commit;
insert into SITE_CONFIG (CONFIG_KEY, CONFIG_VALUE, CONFIG_COMMENT, CONFIG_REQUIRED_BY_GUI) 
values ('SITE_OO_FILTER_BY_EXPIRY', 'N', 'determine if open orders are filtered by expiry date or ordered date', NULL );
commit;
