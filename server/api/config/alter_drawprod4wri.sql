/*
    Add the following new column in table PRODUCTS for the feature of WRI numbers:
    PROD_WRI_REQD: Is WRI number required for the product in an open order? (Y/N)
*/

alter table PRODUCTS add PROD_WRI_REQD NUMBER(1);

COMMIT;


-- update GUI_PRODUCTS to add new column
CREATE OR REPLACE FORCE VIEW GUI_PRODUCTS AS
  select
    ...
    , prod.PROD_WRI_REQD            as PROD_WRI_REQD
