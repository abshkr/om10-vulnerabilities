    ...
    public function read()
    {
        ...
        FROM 
            ...
            SELECT LOCKAL.LOCKATYP_AT_CMPY,
                LOCKAL.LOCKAL_SUPL,
                NVL(LISTAGG(LOCKAL.LOCKAL_LOCK, ', ') WITHIN GROUP (ORDER BY LOCKAL.LOCKAL_LOCK), ' ') as LOCKAL_LOCK,
                NVL(LISTAGG(ALLOC_LOCK_TYP.ALLOC_LOCK_NAME, ', ') WITHIN GROUP (ORDER BY LOCKAL.LOCKAL_LOCK), ' ') as ALLOC_LOCK_NAME
            FROM LOCKAL, ALLOC_LOCK_TYP
            WHERE LOCKAL.LOCKATYP_AT_TYPE = 3
                AND LOCKAL.LOCKAL_LOCK= ALLOC_LOCK_TYP.ALLOC_LOCK_ID
            GROUP BY LOCKAL.LOCKATYP_AT_CMPY, LOCKAL.LOCKAL_SUPL
            ) AL,
            (
            SELECT ORDER_CUST,
