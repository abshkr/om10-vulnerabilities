...
class SiteBal extends CommonClass
{
    ...
    public function read_with_base_change()
    {
        ...
        WHERE CLOSEOUTS.CLOSEOUT_NR = :cls_out
            ...
            AND TANKS.TANK_TERMINAL = TRANSFER_OUT_INFO.TRSB_TK_TANKDEPO(+)
            AND TANKS.TANK_TERMINAL = RACK_INFO.TRSB_TK_TANKDEPO(+)
            AND NVL(CLOSEOUT_TANK.TANK_BASECODE, TANKS.TANK_BASE) = RECEIPT_INFO.TRSB_BS(+)
            AND NVL(CLOSEOUT_TANK.TANK_BASECODE, TANKS.TANK_BASE) = TRANSFER_IN_INFO.TRSB_BS(+)
            AND NVL(CLOSEOUT_TANK.TANK_BASECODE, TANKS.TANK_BASE) = TRANSFER_OUT_INFO.TRSB_BS(+)
            AND NVL(CLOSEOUT_TANK.TANK_BASECODE, TANKS.TANK_BASE) = RACK_INFO.TRSB_BS(+)
            AND TANKS.TANK_TERMINAL = TERMINAL.TERM_CODE
            $terminal_condition
