...
class SiteConfig extends CommonClass
{
    ...
    public function update() 
    {
        ...
        if ($this->config_key === "SITE_AL_ADJ" ||
            ...
            $query = "SELECT SITE_AL_ADJ, SITE_CL_ADJ, SITE_KG_ADJ FROM SITE";
            $stmt = oci_parse($this->conn, $query);
            if (!oci_execute($stmt, $this->commit_mode)) {
                $e = oci_error($stmt);
                write_log("DB error:" . $e['message'], __FILE__, __LINE__, LogLevel::ERROR);
            ...
            if ($this->config_key === "SITE_AL_ADJ") {
                $query = "UPDATE SITE SET SITE_AL_ADJ = :new_adjustment";
            } else if ($this->config_key === "SITE_CL_ADJ") {
                $query = "UPDATE SITE SET SITE_CL_ADJ = :new_adjustment";
            } if ($this->config_key === "SITE_KG_ADJ") {
                $query = "UPDATE SITE SET SITE_KG_ADJ = :new_adjustment";
            }
