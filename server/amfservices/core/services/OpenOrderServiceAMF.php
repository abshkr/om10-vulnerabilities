...
/*
		...
		$count = $mydb->count( $query );
		// get the records
        $rows = $mydb->query( $queryPaged );        
...
//                return "ERROR";
				...
				$data->sess0 = $sess_obj->getSessionVar('SESSION')
						."---".$sess_obj->getSessionVar('MANAGER')
						."---".$sess_obj->getSessionVar('LANGUAGE')
						."---".$sess_obj->getSessionVar('COMPANY')
						."---".$sess_obj->getSessionVar('SITECODE')
						."---".$sess_obj->getSessionVar('PERCODE')
						."---".$sess_obj->getSessionVar('PERNAME')
				;
		
				$data->sess1 = $data->session_id;
				$data->sess2 = $_SESSION;
				$data->sess3 = $_SERVER;
				$data->host = $this->host;
				$data->server = $_SERVER['HTTP_HOST'];
				$data->server2 = $_SERVER['SERVER_NAME'];
				$data->cgi = $this->cgi;
				$data->rtnresult = $response;
				return $data;
				*/
					return "ERROR";
				}
