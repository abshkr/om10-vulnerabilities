...
const OrderListings = ({ popup, params }) => {
  ...
  const [timeOptionSearch, setTimeOptionSearch] = useState(filterByExpiry ? 'ORDER_EXP_TIME' : 'ORDER_ORD_TIME');
  ...
  const [mainUrl, setMainUrl] = useState(popup && supplier && customer
    ? `${ORDER_LISTINGS.READ}?start_date=${!start?'-1':start}&end_date=${!end?'-1':end}&time_option=${timeOption}&order_supp_code=${supplier}&order_cust_acnt=${customer}`
    : `${ORDER_LISTINGS.READ}?start_date=${!start?'-1':start}&end_date=${!end?'-1':end}&time_option=${timeOption}`
  );
  ...
  const onRefresh = () => {
    ...
    // );
    const tempUrl = (popup && supplier && customer
      ? `${ORDER_LISTINGS.READ}?start_date=${!start?'-1':start}&end_date=${!end?'-1':end}&time_option=${timeOption}&order_supp_code=${supplier}&order_cust_acnt=${customer}&order_stat_id=${''}&order_cust_no=${''}&order_ref_code=${''}`
      : `${ORDER_LISTINGS.READ}?start_date=${!start?'-1':start}&end_date=${!end?'-1':end}&time_option=${timeOption}&order_supp_code=${''}&order_cust_acnt=${''}&order_stat_id=${''}&order_cust_no=${''}&order_ref_code=${''}`
    );
    setMainUrl(tempUrl);
  ...
  const runSearch = (values) => {
    ...
    setTimeOptionSearch(values.use_date_range ? values.time_option : (filterByExpiry ? 'ORDER_EXP_TIME' : 'ORDER_ORD_TIME'));
    setUseSearch(true);
    ...
    const timeOptionSearch = (values.use_date_range ? values.time_option : (filterByExpiry ? 'ORDER_EXP_TIME' : 'ORDER_ORD_TIME'));
    // const tempUrl = (popup && supplier && customer
