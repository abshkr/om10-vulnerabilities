...
const Axles = ({ form, value }) => {
  ...
  const fetchAxles = useCallback(
    (supp, trip) => {
      ...
      api
        .get(`${AXLE_WEIGHTS.GET_TRIP_AXLE_WEIGHTS}?supplier_code=${supp}&shls_trip_no=${trip}`)
        .then((response) => {
  ...
  useEffect(() => {
    console.log('.........edit1 trip', value, data);
    if (value) {
      console.log('.........edit2 trip', value, data);
      fetchAxles(value?.supplier_code, value?.shls_trip_no);
    }
  }, [value, fetchAxles]);
