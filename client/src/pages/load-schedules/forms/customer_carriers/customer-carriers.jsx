...
const CustomerCarriers = ({ customer, disabled }) => {
  ...
  const leftTableColumns = [
    {
      title: t('fields.carrierCode'),
      dataIndex: 'cmpy_code',
    },
    {
      title: t('fields.carrierName'),
      dataIndex: 'cmpy_name',
    },
  ];
  ...
  const { data: allCarriers, isValidating } = useSWR(`${CUSTOMERS.ALL_CARRIERS}?customer=${customer}`, {
    refreshInterval: 0,
  ...
  let data = allCarriers?.records;

  useEffect(() => {
    if (!!allCarriers) {
      data = allCarriers?.records;
    }
  }, [allCarriers]);

  /*   const { data: allCarriers } = useSWR(
    `${CUSTOMERS.ALL_CARRIERS}?customer=${customer}`,
    { refreshInterval: 0,
    }
  );
  ...
  */
  ...
  const createLinksForEach = async (keys) => {
    ...
    items.forEach((item) => {
      const link = {
        cust_acct: customer,
        cmpy_code: item?.cmpy_code,
      };
      console.log('item:', item, link);
      api
        .post(CUSTOMERS.CREATE_CUSTOMER_CARRIER, link)
        .then(() => {
          mutate(`${CUSTOMERS.ALL_CARRIERS}?customer=${customer}`);
  ...
  const deleteLinksForEach = async (keys) => {
    ...
    items.forEach((item) => {
      ...
      console.log('item:', item, link);
      api
        .post(CUSTOMERS.DELETE_CUSTOMER_CARRIER, link)
        .then(() => {
          mutate(`${CUSTOMERS.ALL_CARRIERS}?customer=${customer}`);
          notification.success({
  ...
  const createLinks = async (keys) => {
    ...
    api
      .post(CUSTOMERS.CREATE_CUSTOMER_CARRIERS, links)
      .then(() => {
        mutate(`${CUSTOMERS.ALL_CARRIERS}?customer=${customer}`);
        notification.success({
  ...
  const deleteLinks = async (keys) => {
    ...
    api
      .post(CUSTOMERS.DELETE_CUSTOMER_CARRIERS, links)
      .then(() => {
        mutate(`${CUSTOMERS.ALL_CARRIERS}?customer=${customer}`);
        notification.success({
  ...
  const linkTitle = t('fields.custCarrierLinks');
  // const leftTitle = (<div style={{fontSize: 14, fontWeight: 'bold', color: 'black'}}>{t('fields.custAvailableCarriers')}</div>);
  // const rightTitle = (<div style={{fontSize: 14, fontWeight: 'bold', color: 'black'}}>{t('fields.custLinkedCarriers')}</div>);
  const leftTitle = (
    <Tag color={'red'}>
      <div style={{ fontWeight: 'bold' }}>{t('fields.custAvailableCarriers')}</div>
    </Tag>
  );
  const rightTitle = (
    <Tag color={'red'}>
      <div style={{ fontWeight: 'bold' }}>{t('fields.custLinkedCarriers')}</div>
    </Tag>
  );
  ...
  return (
    <TableTransfer
      ...
      filterOption={(inputValue, item) =>
        item.cmpy_name.indexOf(inputValue) !== -1 || item.cmpy_code.indexOf(inputValue) !== -1
      }
