...
const buildPayloadToRepost = (transaction) => {
    ...
    for (tidx = 0; tidx < tcount; tidx++) {
      ...
      */
      ...
      transfer.trsf_drwr_cd = transaction?.drawer;
      transfer.trsf_prod_cmpy = transaction?.drawer;
    ...
    */
    ...
    _.forEach(values?.base_transfers, (item) => {
      ...
      for (let index = 0; index < values?.base_totals?.length; index++) {
        ...
        if (total?.trsf_bs_prodcd_tot === item?.trsf_bs_prodcd && total?.trsf_bs_tk_cd_tot === item?.trsf_bs_tk_cd) {
          total.trsf_bs_qty_amb_tot = _.toNumber(total?.trsf_bs_qty_amb_tot) + _.toNumber(item?.trsf_bs_qty_amb);
          total.trsf_bs_qty_cor_tot = _.toNumber(total?.trsf_bs_qty_cor_tot) + _.toNumber(item?.trsf_bs_qty_cor);
          total.trsf_bs_load_kg_tot = _.toNumber(total?.trsf_bs_load_kg_tot) + _.toNumber(item?.trsf_bs_load_kg);
