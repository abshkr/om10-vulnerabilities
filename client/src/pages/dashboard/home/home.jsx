...
const Home = () => {
  ...
  const onID = (v) => {
    if (v.target.className === 'ant-card-head-title') {
      navigate(ROUTES.ID_ASSIGNMENT);
    }
  };

  const onFolio = (v) => {
    if (v.target.className === 'ant-card-head-title') {
      navigate(ROUTES.FOLIO_SUMMARY);
    }
  };

  const onSchedule = (v) => {
    if (v.target.className === 'ant-card-head-title') {
      navigate(ROUTES.LOAD_SCHEDULES);
    }
  };

  const onTransactionList = (v) => {
    if (v.target.className === 'ant-card-head-title') {
      navigate(ROUTES.TRANSACTION_LIST);
    }
  };
  ...
  return (
    <>
      <Row gutter={[16, 16]}>
        <Col span={8}>
          <Card
            title={t('pageNames.overviewTankers')}
            hoverable
            size="small"
            loading={isLoading}
            onClick={onTankers}
        ...
        <Col span={8}>
          <Card
            title={t('pageNames.overviewPersonnel')}
            hoverable
            size="small"
            loading={isLoading}
            onClick={onPersonnel}
        ...
        <Col span={8}>
          <Card
            title={t('pageNames.overviewIdAssignment')}
            hoverable
            size="small"
            loading={isLoading}
            onClick={onID}
          >
            ...
            <div style={{ fontSize: 16, position: 'absolute', bottom: 5, fontWeight: 500 }}>
              {`${t('fields.countTagPersonnel')}: ${data?.key_person || 0}, ${t('fields.countTagTanker')}: ${
                data?.key_tanker || 0
              }, ${t('fields.countTagCombo')}: ${data?.key_combo || 0}, ${t('fields.countTagOther')}: ${
                data?.key_other || 0
              }`}
            </div>
          </Card>
      ...
      <Row gutter={[16, 16]}>
        <Col span={8}>
          <Card
            title={t('pageNames.overviewCurrentFolio')}
            hoverable
            size="small"
            loading={isLoading}
            onClick={onFolio}
        ...
        <Col span={8}>
          <Card
            title={t('pageNames.overviewTankerMovement')}
            hoverable
            size="small"
            loading={isLoading}
            onClick={onSchedule}
        ...
        <Col span={8}>
          <Card
            title={t('pageNames.overviewTransactionNumbers')}
            hoverable
            size="small"
            loading={isLoading}
            onClick={onTransactionList}
          >
