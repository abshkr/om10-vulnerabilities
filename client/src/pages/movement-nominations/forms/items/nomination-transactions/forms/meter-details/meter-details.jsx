    ...
    <>
      <Spin indicator={null} spinning={isLoading}>
        ...
        <Row gutter={[8, 8]}>
          ...
          <Col span={5}>
            <strong>
              {t('fields.nomtranObsTotal')} {_.round(_.sumBy(data, 'trsf_qty_amb'), config?.precisionVolume)}
            </strong>
          </Col>
          <Col span={5}>
            <strong>
              {t('fields.nomtranStdTotal')} {_.round(_.sumBy(data, 'trsf_qty_cor'), config?.precisionVolume)}
            </strong>
          </Col>
          {config?.siteMassInVacuum && (
            <Col span={5}>
              <strong>
                {t('fields.nomtranMassTotal')} {_.round(_.sumBy(data, 'trsf_load_kg'), config?.precisionMass)}
              </strong>
            </Col>
          )}
