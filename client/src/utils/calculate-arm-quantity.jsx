...
const calcArmQuantity = async (stream, arms, amount, type, temperature) => {
  let index = undefined;
  ...
  console.log('----- utils: calcArmQuantity', stream, arms, amount, type, temperature);
  const transfer = {
  ...
  for (index = 0; index < bases.length; index++) {
    ...
    if (base.result) {
      transfer.qty_amb += type !== 'LT' ? base.qty_amb : 0;
      transfer.qty_cor += type !== 'L15' ? base.qty_cor : 0;
      transfer.load_kg += type !== 'KG' ? base.load_kg : 0;
    } else {
