		...
		foreach( $_SESSION['pipeNodesRT'] as $key=>$line )
		{
			...
			foreach( $bay_item as $node_key=>$node_item )
			{
				if ( $node_item['bay_code'] == $key )
				{
					...
					$node_item['qty_in_cmpt'] = round($cmpt_item_scada->qty_in_cmpt);
					$node_item['amb_in_cmpt'] = round($cmpt_item_scada->amb_in_cmpt);
					$node_item['std_in_cmpt'] = round($cmpt_item_scada->std_in_cmpt);
					$node_item['mass_in_cmpt'] = round($cmpt_item_scada->mass_in_cmpt);
	...
	public function lookupAdhocTripCompartments($trip_no, $supp_code, $caseType='L')
	{
        ...
        $sql['sql_text'] = "
            SELECT
				...
				, SPEC_PROD.QTY_SCHEDULED	as QTY_PRESET
